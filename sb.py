...
parser = argparse.ArgumentParser(description='Selfbot DPK Team')
...
def executeCmd(msg, text, txt, cmd, msg_id, receiver, sender, to, setKey):
    if cmd == 'logoutbot':
        ...
        line.sendReplyMessage(msg_id, to, special())
    elif cmd == 'speed':
        start = time.time()
                ...
                if text.lower() == 'dell':
                    ...
                    line.sendMessage(to, "Auto like non actived")
                              ...
                              if sender in myMid:
                                ...
                                data = url.json()
                                result = "╭───[ Masehi to Hijriyah]\n├ Hijriah = {}".format(str(data["data"]["hijri"]["date"]))
                                result += "\n├ Masehi  = {}\n╰───[ Hijriyah to Masehi]".format(str(data["data"]["gregorian"]["date"]))
                                line.sendMessage(to, result)
                elif msg.text.lower().startswith("asmaulhusna "):
                                sep = text.split(" ")
                                txt = text.replace(sep[0] + " ","")
                                url = requests.get("http://api.aladhan.com/asmaAlHusna/{}".format(txt))
                                data = url.json()
                                result = "╭───[ Asmaul Husna ]\n├ Asma Allah ke {} =  {} ".format(txt,data["data"][0]["name"])
                                result += "\n├ Translate = {} \n╰───[ Good name ]".format(data["data"][0]["en"]["meaning"])
                                line.sendMessage(to, result)
                                ...
                                k = len(result)//100000
                                for aa in range(k+1):
                                    line.sendMessage(to,'{}'.format(result[aa*100000 : (aa+1)*100000]))
                            ...
                            if len(count) == 1:
                                ...
                                for aa in data["list"]:
                                    no += 1
                                    ret_ += "\n" + str(no) + ". " + str(aa["title"])
                                ret_ += "\n───────────────\nNext ketik: \n     smule {}-number\nto looking & listening record ".format(str(search))
